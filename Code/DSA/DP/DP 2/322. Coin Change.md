...
    private int recur(int[] arr, int idx , int sum){
...
        int stay = recur(arr,idx , sum-arr[idx])+1; // +1 because 1 coin used.

        int move = recur(arr,idx+1,sum); // no coin used.

  

        return Math.min(stay,move); // minimum coin used which part.
